...
## `unique_species` and the element balance — the guards on the dialog with
...
#
# Guarded like the other two: `run_5.jl` includes this file for its OPC initialization and
...
function _compute_opc_ic4(
    ...
    V_liq_eq < 1.0e-15 && error("Zero liquid volume after the OPC IC equilibrate")
    ...
    return (
        phi = phi0, n_ch = n_ch_eq, n_ett = n_ett_eq, n_ms = n_ms_eq, n_fs = n_fs_eq,
...
Constructor with thermodynamic OPC initialization + initial DLM.
...
function PoroMechanics.storage!(f, u, node, m::ChlorideModel4, ::Any)
    ...
    return f[ICA4] = (phi + m.Kd_Ca[i]) * u[ICA4]
end
...
function PoroMechanics.flux!(f, u, edge, m::ChlorideModel4, ::Any)
    ...
    return f[ICA4] = m.D_Ca * tau * (u[ICA4, 1] - u[ICA4, 2])
end
...
function PoroMechanics.bcondition!(f, u, bnode, m::ChlorideModel4, ::Any)
    ...
    return boundary_dirichlet!(f, u, bnode; species = ICA4, region = 1, value = m.c_ca_BC)
end
...
"""
    certified_transient_equilibrium4(state)

Return a certified equilibrium, including sensitivities for dual-valued amounts.
Reject failed certification before the transient step consumes the composition.
"""
function certified_transient_equilibrium4(state; solve_equilibrium = ChemistryLab.equilibrate_certified)
    equilibrium, certificate = solve_equilibrium(state)
    certificate !== nothing && certificate.optimal ||
        error("Transient chemical equilibrium is not certified: $certificate")
    return equilibrium
end
...
For every interior node (i ≥ 2):
  ...
  2. Certify the Gibbs equilibrium against the input component totals.
...
function chemistry_step4!(m::ChlorideModel4, u::Matrix, cs, has_friedels::Bool)
    ...
    for i in 2:N
        ...
        state_eq = certified_transient_equilibrium4(state)

        V_liq = ustrip(uconvert(us"m^3", state_eq.V_phases[].liquid))
        isfinite(V_liq) && V_liq >= 1.0e-15 || error("Invalid liquid volume at node $i: $V_liq")
        ...
        m.c_oh_frozen[i] = max(c_oh_new, 1.0e-12)
        ...
        # A failed certified sensitivity aborts the segment, as a failed primal solve does.
        dn_fs_dc = 0.0
        if has_friedels
            state_eq_t = certified_transient_equilibrium4(
...
function run_chloride_ingress4(;
    ...
    let phi0 = 0.121
        ...
        D_eff = 2.032e-9 * tau
        front_1yr = 2 * sqrt(D_eff / phi0 * t_end) * 1.0e3
    ...
    results = Tuple{
        Float64, Matrix{Float64}, Vector{Float64}, Vector{Float64},
    ...
    for k in 2:lastindex(tsave)
        ...
        @info "SNIA" seg = k - 1 φ_mean = round(mean(m.phi); sigdigits = 4) n_FS0 = round(m.n_fs[2]; sigdigits = 4) S_Cl0 = round(m.S_Cl[2]; sigdigits = 4) β_node2 = round(
            let (_b, _, _, _, _, _) = solve_dlm(max(u_cur[ICL4, 2], 0.0), max(u_cur[INA4, 2], 0.0), max(u_cur[IK4, 2], 0.0), max(u_cur[ICA4, 2], 0.0), 0.0, max(m.c_oh_frozen[2], 1.0e-15), m.dlm.n_csh0, m.dlm.x_cas; dlm = m.dlm, T_K = m.T_K)
                _b
            end; sigdigits = 3
        )
    end
...
function compare_reference_4(results, grid; dlm::DLM = DLM_TRAN2018())
    ...
    x_ref = [0.0, 0.005, 0.01, 0.015, 0.02, 0.03, 0.04, 0.05, 0.065, 0.075]
    ...
    for (xr, cr) in zip(x_ref, c_ref)
        ...
        try
            β_node, = solve_dlm(
                ...
                max(u[ICA4, idx], 0.0), 0.0, max(c_oh[idx], 1.0e-15),
                dlm.n_csh0, dlm.x_cas; dlm = dlm, T_K = 293.15
            )
        catch
        end
        @printf(
            "%-8.4f  %-14.4f  %-10.4f  %-12.4f  %-10.4f  %-8.2f  %-10.2f  %-10.3f\n",
            xr, C_Cl_dm3[idx], cr, c_oh_dm3[idx], phi[idx], n_fs[idx], S_Cl[idx], β_node
        )
    end
    println("="^115)

    idx_front = findlast(C_Cl_dm3 .> 1.0e-3)
    ...
    println("  " * "-"^60)
    for xr in [0.005, 0.01, 0.02, 0.03, 0.05]
        idx = argmin(abs.(x_dm .- xr))
...
function plot_chloride_ingress4(results, grid; n_curves = 4, save_path = nothing, dlm::DLM = DLM_TRAN2018())
    ...
    n_ref_total = [
        0.16614, 4.16643, 4.03841, 1.55713, 0.08571, 0.0829,
        0.07986, 0.0765, 0.07265, 0.06798, 0.06189, 0.05317,
        0.03986, 0.02237, 0.00885, 0.00301,
    ]  # mol/dm³_concrete
    ...
    c_ref_g100 = [
        c_ref[i] * 1000.0 * phi_f[argmin(abs.(x_dm .- x_ref[i]))] * fac
            for i in eachindex(x_ref)
    ]

    # ── p1: Cl split at the final time ────────────────────────────────────────
    p1 = plot(;
        xlabel = "Profondeur x [dm]",
    ...
    for (k, ti) in enumerate(idxs)
        ...
        plot!(
            p2, x_dm, C_tot_k; lw = 2, color = pal[mod1(k, end)],
    ...
    for (k, ti) in enumerate(idxs)
        ...
        phi_k = results[ti][3]
        plot!(
            p3, x_dm, phi_k; lw = 2, color = pal[mod1(k, end)],
            label = "t = $(round(t_k / t_yr; digits = 2)) yr"
        )
    end
    ...
    β_profile = [
        begin
            β_v, = solve_dlm(
                max(u_f[ICL4, j], 0.0), max(u_f[INA4, j], 0.0), max(u_f[IK4, j], 0.0),
                max(u_f[ICA4, j], 0.0), 0.0, max(c_oh_f[j], 1.0e-15),
                dlm.n_csh0, dlm.x_cas; dlm = dlm, T_K = 293.15
            )
            β_v
